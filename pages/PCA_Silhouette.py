...
# ── Header ─────────────────────────────────────────────────────────────────────
st.markdown('<h1 class="title">Cluster Banjir Pada Bandung Raya (PCA Silhouette)</h1>', unsafe_allow_html=True)
if main_image:
...
with left_col:
    ...
    scatter_df = df.copy()
    scatter_df['cluster_label'] = scatter_df['cluster_sil'].astype(str).apply(lambda x: f"Cluster {x}")
...
with right_col:
    ...
    karakteristik_df = (
        df.groupby('cluster_sil')
        .agg(agg_dict)
        .round(3)
        .reset_index()
        .rename(columns={'cluster_sil': 'cluster'})
    )
...
for col, year in zip(map_cols, years):
    with col:
        ...
        year_kec = (
            df[df['year'] == year]
            .groupby('kecamata')['cluster_sil']
            .agg(lambda x: x.mode()[0])
            .reset_index()
        )
        year_kec['cluster_label'] = year_kec['cluster_sil'].astype(str).apply(lambda x: f"Cluster {x}")
...
CLUSTER_DESC = {
    0: {"Label": "Rawan",       "Deskripsi": "Dataran rendah"},
    1: {"Label": "Tidak Rawan", "Deskripsi": "Dataran tinggi"},
}
